...
- Enemy waves that grow over time
...
- [Enemy.java](Enemy.java) - Defines enemy movement, collision, damage, knockback, death behavior, and enemy rendering.
- [EnemyManager.java](EnemyManager.java) - Stores and updates all enemies currently active.
- [WaveManager.java](WaveManager.java) - Spawns enemy waves and increases wave difficulty over time.
...
- Enemies spawn from outside the window edges. The first wave contains 3 enemies; each subsequent wave adds 2 more.
- When all enemies in a wave are dead, a 3-2-1-START countdown plays before the next wave spawns.
